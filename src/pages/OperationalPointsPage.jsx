    ...
    <RoleShell variant="operacional" contentClassName="px-4 py-4 pb-28 sm:px-6 sm:py-6 lg:px-8">
      <div className="space-y-5">
        <PageHeader
          ...
          description="Mantenha os dados publicos e operacionais dos pontos sob sua responsabilidade."
        ...
        {!pointsQuery.isLoading && !points.length ? <EmptyState title="Nenhum ponto vinculado a sua conta." icon={MapPin} /> : null}

        {selected && form ? (
          <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
            <SectionCard title="Pontos vinculados">
            ...
            <SectionCard title="Dados do ponto" description="As alteracoes ficam visiveis no mapa e nos fluxos de descarte.">
              <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2"><Field label="Endereco" value={form.endereco} onChange={(value) => updateField("endereco", value)} /></div>
                <Field label="Capacidade maxima (kg)" type="number" value={form.capacidade_maxima} onChange={(value) => updateField("capacidade_maxima", value)} />
                <Field label="Raio de operacao (m)" type="number" value={form.raio_operacao} onChange={(value) => updateField("raio_operacao", value)} />
                <div className="sm:col-span-2"><Field label="Materiais aceitos, separados por virgula" value={form.tipos_residuos_aceitos} onChange={(value) => updateField("tipos_residuos_aceitos", value)} /></div>
                <div className="sm:col-span-2"><Field label="Horario de funcionamento" value={form.horario_funcionamento} onChange={(value) => updateField("horario_funcionamento", value)} /></div>
                {feedback ? <div className="sm:col-span-2"><InlineAlert variant={feedback.type} description={feedback.text} /></div> : null}
                <Button type="submit" disabled={saveMutation.isPending} className="sm:col-span-2 sm:justify-self-end"><Save className="mr-2 h-4 w-4" /> {saveMutation.isPending ? "Salvando..." : "Salvar alteracoes"}</Button>
              </form>
            </SectionCard>
          </div>
        ) : null}
...
function Field({ label, value, onChange, type = "text", as, options = [] }) {
